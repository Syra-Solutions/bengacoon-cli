...
export function orchestratorPrompt(cwd, selectedTools, dir = ORCHESTRATOR_DIR, scriptsDir = SCRIPTS_DIR) {
  ...
  const deliveryReporter = Array.isArray(selectedTools) && selectedTools.includes("bengacoon_report_delivery")
    ...
    : undefined;
  const deliveryMirror = Array.isArray(selectedTools) && selectedTools.includes("bengacoon_report_delivery") && selectedTools.some((name) => String(name).includes("mem_save"))
    ? "After reporting delivery, mirror its work item, verification, and next step with mem_save under topic_key delivery/current-work. This mirror is context only: never record a receipt there or treat it as delivery authority."
    : undefined;
  const reviewReporter = Array.isArray(selectedTools) && selectedTools.includes("bengacoon_report_review")
    ...
    : undefined;
  return [rule, language, voice, contextStoreAdapter(selectedTools, dir), rule ? scriptsLocation(scriptsDir) : undefined, deliveryReporter, deliveryMirror, reviewReporter]
    .filter(Boolean)
    .join("\n\n");
}
