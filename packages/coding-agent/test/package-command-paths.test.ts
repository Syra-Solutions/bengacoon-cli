...
const bengacoonPath = path.join(binDir, process.platform === "win32" ? "bengacoon.cmd" : "bengacoon");
fs.writeFileSync(
	bengacoonPath,
...
if (process.platform !== "win32") fs.chmodSync(bengacoonPath, 0o755);
`,
		);
	...
	it("skips untrusted project package settings", async () => {
		mkdirSync(join(projectDir, ".bengacoon"), { recursive: true });
		writeFileSync(
			join(projectDir, ".bengacoon", "settings.json"),
	...
	it("blocks local package changes when project is untrusted", async () => {
		mkdirSync(join(projectDir, ".bengacoon"), { recursive: true });
		writeFileSync(join(projectDir, ".bengacoon", "settings.json"), "{}");
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		const settingsPath = join(projectDir, ".bengacoon", "settings.json");
		const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "";
		expect(realpathSync(join(projectDir, ".bengacoon", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("bengacoon install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain(
				'Use "bengacoon --help" or "bengacoon install <source> [-l] [--approve|--no-approve]".',
			);
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: bengacoon install <source> [-l]");
	...
	it("allows explicit self-update checks when automatic version checks are disabled", async () => {
		...
		try {
			...
			expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
				`bengacoon is already up to date (v${VERSION})`,
			);
			expect(errorSpy).not.toHaveBeenCalled();
			expect(process.exitCode).toBeUndefined();
		} finally {
	...
	it("updates installer-managed Bengacoon through a staged immutable release", async () => {
		...
		expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
			`Updated bengacoon from ${VERSION} to ${targetVersion}`,
		);
		expect(errorSpy).not.toHaveBeenCalled();
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("rejects a concurrent managed update", async () => {
		...
		expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).not.toContain("Updated bengacoon from");
		expect(errorSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
	...
	it("rejects forced managed reinstalls", async () => {
		...
		expect(existsSync(npmRecordPath)).toBe(false);
		expect(errorSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
			"Managed bengacoon installations do not support --force",
		);
		expect(process.exitCode).toBe(1);
	});
...
`,
		...
		try {
			...
			expect(stdout).toContain(`Updated bengacoon from ${VERSION} to ${VERSION}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		try {
			...
			expect(recordedArgs).not.toContain(PACKAGE_NAME);
			expect(stdout).toContain(`Updated bengacoon from ${VERSION} to ${targetVersion}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
	...
	it("prints a pnpm metadata hint when self-update fails", async () => {
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).not.toContain("Updated bengacoon");
			expect(stderr).toContain("exited with code 23");
			expect(stderr).toContain("If pnpm reports missing package versions");
			expect(stderr).toContain("Run `pnpm store prune` and retry `bengacoon update --self`.");
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		...
		try {
			...
			expect(stdout).not.toContain("Updated bengacoon-formatter");
			expect(process.exitCode).toBe(1);
