...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".bengacoon", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".bengacoon", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".bengacoon", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".bengacoon"), { recursive: true });
			writeFileSync(join(cwd, ".bengacoon", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".bengacoon"), { recursive: true, force: true });
